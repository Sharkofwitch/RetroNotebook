...
## [1.4.0] – 2026 (Version History & Restore Points)

### Added
- **Notebook version history** – automatic snapshots are taken on every manual
  save and before any restore operation.
- **`app/history.py`** – new module with:
  - `save_snapshot(cells, name, reason)` – serializes the full notebook and
    writes a microsecond-timestamped JSON file under
    `~/retro-notebook-notebooks/.history/<notebook_name>/`.
  - `list_snapshots(name)` – returns all snapshots newest-first.
  - `get_diff_summary(snap, current)` – produces a human-readable diff
    (e.g. `+2 cells, ~1 edited`).
  - `cleanup_old_snapshots(name, keep=20)` – enforces a 20-snapshot
    retention limit; oldest files are deleted automatically.
- **`HistoryDialog`** – retro-styled history browser accessible via the new
  `🕑 History` toolbar button.
  - Left pane: chronological snapshot list with timestamp, reason icon
    (💾 Save / ↩ Pre-restore), and cell count.
  - Right pane: per-cell preview, color-coded by type (Code/Markdown/Test).
  - Diff bar: shows changes from the selected snapshot to the current state.
  - **Restore** creates a `"pre-restore"` snapshot first, making every
    restore fully reversible.
- `cells_to_data(cells)` helper extracted from `save_notebook` in
  `storage.py` and reused by the history module.
- Save/Load/History buttons are now displayed in a single horizontal row.
- `on_save()` briefly flashes **Gespeichert ✓** on the status LED after a
  successful save.

### Fixed
- **Notebook save was silently broken.** `NOTEBOOK_FILE` was set to
  `"notebooks/auto_save.json"`, which resolved to a subdirectory that was
  never created.  
  Fixed by:
  - Simplifying the filename to `"auto_save.json"`.
  - Updating `storage.get_notebook_path()` to call
    `os.makedirs(os.path.dirname(path), exist_ok=True)` so the full
    directory tree is always created before writing.
